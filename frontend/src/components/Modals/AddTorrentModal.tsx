...
import { X, Upload, Link as LinkIcon, File as FileIcon, Check } from 'lucide-react';
...
const AddTorrentModal: React.FC<AddTorrentModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen && currentContainerId) {
      ...
      axios.get(`/api/torrents/categories?containerId=${currentContainerId}`)
        .then(res => {
      ...
      axios.get(`/api/preferences?containerId=${currentContainerId}`)
        .then(res => {
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} className="w-full max-w-2xl">
      <div className="bg-white dark:bg-[#1C1C1E] rounded-xl shadow-2xl border border-gray-200 dark:border-gray-800 transition-colors duration-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1C1C1E]">
          <h2 className="text-lg font-medium text-gray-900 dark:text-white">{t('add_torrent.title')}</h2>
        ...
        {/* Content */}
        <div className="p-6 space-y-6 bg-white dark:bg-[#1C1C1E]">
          {activeTab === 'url' ? (
            <div>
              <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{t('add_torrent.urls_label')}</label>
              <textarea
                value={urls}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{t('add_torrent.files_label')}</label>
              <div 
              ...
              >
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('add_torrent.drop_text')}</span>
                <input
                  ref={fileInputRef}
                  type="file"
                  multiple
              ...
              {files.length > 0 && (
                <div className="mt-4 space-y-2">
                  {files.map((file, i) => (
                    <div key={i} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700">
                      <span className="truncate">{file.name}</span>
                      <button onClick={() => setFiles(files.filter((_, idx) => idx !== i))} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="relative">
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('add_torrent.savepath')}</label>
              <input
                type="text"
                value={options.savepath}
                onChange={(e) => setOptions({...options, savepath: e.target.value})}
              ...
              {showSavePathDropdown && existingSavePaths.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg max-h-60 overflow-y-auto">
                  {existingSavePaths.filter(p => p.toLowerCase().includes(options.savepath.toLowerCase())).map(path => (
                    <div
                      key={path}
                      className="px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                      onClick={() => setOptions({...options, savepath: path})}
                    >
                      {path}
                    </div>
                  ))}
            ...
            <div className="relative">
              ...
              {showCategoryDropdown && existingCategories.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg max-h-60 overflow-y-auto">
                  {existingCategories.filter(c => c.toLowerCase().includes(options.category.toLowerCase())).map(cat => (
                    <div
                      key={cat}
                      className="px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                ...
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg max-h-60 overflow-y-auto">
                  {existingTags.map(tag => (
                    <div
                      ...
                      onClick={() => {
                        const currentTags = options.tags.split(',').map(t => t.trim()).filter(Boolean);
                        if (!currentTags.includes(tag)) {
                          setOptions({...options, tags: [...currentTags, tag].join(', ')});
                        } else {
                          setOptions({...options, tags: currentTags.filter(t => t !== tag).join(', ')});
                        }
                      }}
                    >
                      <span className={clsx("w-4 h-4 mr-2 border rounded flex items-center justify-center", options.tags.split(',').map(t => t.trim()).filter(Boolean).includes(tag) ? "bg-blue-500 border-blue-500" : "border-gray-400")}>
                        {options.tags.split(',').map(t => t.trim()).filter(Boolean).includes(tag) && <Check className="w-3 h-3 text-white" />}
                      </span>
                      {tag}
                    </div>
                  ))}
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={(e) => setOptions({...options, paused: e.target.checked})}
            ...
            />
            <label htmlFor="paused" className="ml-2 text-sm text-gray-600 dark:text-gray-400">{t('add_torrent.paused')}</label>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-end space-x-3 rounded-b-xl">
          ...
          <button
            ...
            disabled={loading || (activeTab === 'url' && !urls) || (activeTab === 'file' && files.length === 0)}
