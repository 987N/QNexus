...
const TorrentTable: React.FC<TorrentTableProps> = ({ 
  ...
  const getStateColor = (state: string) => {
    ...
    if (state.includes('error') || state.includes('missing')) return 'text-red-600 dark:text-red-400';
    if (state.includes('paused') || state.includes('stalled')) return 'text-gray-500 dark:text-gray-400';
    if (state.includes('downloading') || state.includes('DL')) return 'text-green-600 dark:text-green-400';
    if (state.includes('uploading') || state.includes('UP')) return 'text-blue-600 dark:text-blue-400';
    if (state.includes('checking')) return 'text-yellow-600 dark:text-yellow-400';
    return 'text-gray-500 dark:text-gray-400';
  };

  // --- Columns Configuration ---
  // Define all possible columns with their render logic
  const columnDefs: Record<string, ColumnDef> = {
    name: {
      ...
      render: (t) => (
        <div className="flex items-center space-x-2 overflow-hidden">
          <span className={clsx('text-lg shrink-0', getStateColor(t.state || ''))}>
            {getStateIcon(t.state || '')}
          </span>
          <span className="truncate text-gray-900 dark:text-gray-200" title={t.name || ''}>{t.name || 'Unknown'}</span>
        </div>
      )
    },
    size: {
      ...
      width: 80,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatSize(t.size || 0)}</span>
    },
    progress: {
      id: 'progress',
    ...
    dlspeed: {
      ...
      width: 90,
      minWidth: 70,
      render: (t) => <span className="text-green-600 dark:text-green-400">{formatSpeed(t.dlspeed || 0)}</span>
    },
    upspeed: {
      id: 'upspeed',
      ...
      width: 90,
      minWidth: 70,
      render: (t) => <span className="text-blue-600 dark:text-blue-400">{formatSpeed(t.upspeed || 0)}</span>
    },
    eta: {
      id: 'eta',
      label: t('table.eta'),
      width: 80,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatETA(t.eta || 0)}</span>
    },
    ratio: {
      id: 'ratio',
      label: t('table.ratio'),
      width: 70,
      minWidth: 50,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatRatio(t.ratio || 0)}</span>
    },
    downloaded: {
      id: 'downloaded',
      ...
      width: 80,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatSize(t.downloaded || 0)}</span>
    },
    uploaded: {
      id: 'uploaded',
      ...
      width: 80,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatSize(t.uploaded || 0)}</span>
    },
    seeds: {
      id: 'seeds',
      label: t('table.seeds'),
      width: 70,
      minWidth: 50,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{`${t.num_seeds || 0} (${t.num_complete || 0})`}</span>
    },
    peers: {
      id: 'peers',
      label: t('table.peers'),
      width: 70,
      minWidth: 50,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{`${t.num_leechs || 0} (${t.num_incomplete || 0})`}</span>
    },
    added_on: {
      id: 'added_on',
      ...
      width: 140,
      minWidth: 100,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{formatTime(t.added_on || 0)}</span>
    },
    tracker: {
      id: 'tracker',
      ...
      width: 150,
      minWidth: 100,
      render: (t) => <span className="truncate text-gray-700 dark:text-gray-300" title={t.tracker || ''}>{t.tracker || ''}</span>
    },
    save_path: {
      id: 'save_path',
      ...
      width: 150,
      minWidth: 100,
      render: (t) => <span className="truncate text-gray-700 dark:text-gray-300" title={t.save_path || ''}>{t.save_path || ''}</span>
    },
    category: {
      id: 'category',
      ...
      width: 100,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{t.category || ''}</span>
    },
    tags: {
      id: 'tags',
      label: t('table.tags'),
      width: 100,
      minWidth: 60,
      render: (t) => <span className="text-gray-700 dark:text-gray-300">{t.tags || ''}</span>
    }
  };
  ...
  const handleRowClick = (e: React.MouseEvent, hash: string) => {
    ...
    if (isRangeSelect && lastSelectedHash.current && torrents.some(t => t.hash === lastSelectedHash.current)) {
      const lastIndex = torrents.findIndex(t => t.hash === lastSelectedHash.current);
  ...
  const handleContextMenu = (e: React.MouseEvent, hash: string) => {
    ...
    // If the row is not part of the current selection, select it (and deselect others)
    ...
    // Notify parent to open context menu
    if (onRowContextMenu) {
      onRowContextMenu(e, hash);
    }
  };
  ...
  const handleResizeStart = (e: React.MouseEvent, columnId: string, currentWidth: number) => {
    e.preventDefault();
    e.stopPropagation();
    resizingRef.current = { startX: e.pageX, startWidth: currentWidth, columnId };
  ...
  return (
    <div className="flex-1 overflow-auto relative select-none">
      <div 
        ...
        style={{ width: visibleColumns.reduce((acc, col) => acc + (columnWidths[col.id] || col.width), 0) }}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 flex bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          {visibleColumns.map((col) => (
            ...
            >
              ...
              <div
                ...
                onMouseDown={(e) => handleResizeStart(e, col.id, columnWidths[col.id] || col.width)}
          ...
          {torrents.map((torrent) => {
            ...
            return (
              <div
                ...
                onClick={(e) => handleRowClick(e, torrent.hash)}
                onDoubleClick={() => onRowDoubleClick && onRowDoubleClick(torrent.hash)}
                onContextMenu={(e) => handleContextMenu(e, torrent.hash)}
              >
                {visibleColumns.map((col) => (
                  <div
                    key={col.id}
