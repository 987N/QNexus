...
class QBClient {
  constructor(container) {
    ...
    // Handle protocol in host or default to http
    ...
    this.baseUrl = `${protocol}://${host}:${container.port}`;
    this.cookie = null;
    this.appVersion = null;
    this.apiVersion = null;

    // Axios instance with custom agent for self-signed certs
  ...
  async login() {
    try {
      ...
      const response = await this.client.post(
        ...
        )}&password=${encodeURIComponent(this.container.password)}`,
        {
          headers: {
      ...
      // Auto-fallback to HTTPS if HTTP fails with connection issues
      if (
        ...
        console.log("[QBClient] HTTP login failed, trying HTTPS fallback...");
        ...
        try {
          ...
          console.log(`[QBClient] Retry logging in to ${httpsUrl}...`);
          ...
          if (cookies && cookies.length > 0) {
            ...
            console.log(
              "[QBClient] HTTPS Login successful, switching protocol."
            );
          ...
          console.error(
            "[QBClient] HTTPS fallback failed:",
            httpsError.message
          );
  ...
  async fetchVersions() {
    try {
      const [appVerRes, apiVerRes] = await Promise.all([
        this.client.get(`/api/v2/app/version`, {
          headers: { Cookie: this.cookie },
        }),
        this.client.get(`/api/v2/app/webapiVersion`, {
          headers: { Cookie: this.cookie },
        }),
      ]);
      ...
      console.log(
        `[QBClient] Connected to qBittorrent v${this.appVersion} (API v${this.apiVersion})`
      );
    } catch (err) {
  ...
  async performAction(action, hashes, extraParams = {}) {
    ...
    try {
      console.log(
        `[QBClient] Performing action: ${action} on hashes: ${hashes}`,
        extraParams
      );
      ...
      await this.client.post(`/api/v2/torrents/${action}`, params.toString(), {
        headers: {
          Cookie: this.cookie,
          "Content-Type": "application/x-www-form-urlencoded",
          Referer: this.baseUrl,
  ...
  async addTorrent(options) {
    ...
    if (options.seedingTimeLimit)
      form.append("seedingTimeLimit", options.seedingTimeLimit);
  ...
  async getTorrentPeers(hash) {
    ...
    try {
      ...
      const response = await this.client.get(`/api/v2/sync/torrentPeers`, {
        params: { hash, rid: 0 },
        headers: { Cookie: this.cookie },
      });
  ...
  async setFilePriority(hash, id, priority) {
    ...
    try {
      ...
      await this.client.post(`/api/v2/torrents/filePrio`, params.toString(), {
        headers: {
          Cookie: this.cookie,
          "Content-Type": "application/x-www-form-urlencoded",
        },
  ...
  async getLog(lastKnownId = -1) {
    if (!this.cookie) await this.login();
    try {
      const response = await this.client.get(`/api/v2/log/main`, {
  ...
  async deleteTorrents(hashes, deleteFiles = false) {
    ...
    try {
      ...
      await this.client.post(`/api/v2/torrents/delete`, params.toString(), {
        headers: {
          Cookie: this.cookie,
          "Content-Type": "application/x-www-form-urlencoded",
        },
  ...
  async deleteCategory(categories) {
    ...
    try {
      ...
      if (error.response && error.response.status === 403) {
        ...
        await axios.post(
          `${this.baseUrl}/api/v2/torrents/removeCategories`,
          params.toString(),
          {
            headers: {
              Cookie: this.cookie,
